...
import { getGuest, getWishes } from "@/lib/db";
...
function alpineValue(value: unknown) {
...
export async function invitationResponse(slug?: string) {
  const [guest, wishes] = await Promise.all([
    slug ? getGuest(slug) : genericGuest,
    getWishes(),
  ]);
  ...
  const hasRsvp = Boolean(guest.attendance);
  const html = template
    ...
    .replace(
      ...
      `currentPage: 1,\n            lastPage: ${wishes.pages},\n            total: ${wishes.total},\n            items: ${alpineValue(wishes.data)},\n            init()`,
    );
